...
ケースごとの構成の雛形を作るコマンド（§19）。対話式のものは質問に答えて進める。先に共通の準備を済ませておく。
...
```bash
# TypeScript・マネージド（Cloudflare Durable Objects）
pnpm create next-app@latest web
pnpm create cloudflare@latest realtime --type=hello-world-durable-object
# 自前運用（AWS・オンプレ）なら、Node.jsのWebSocketサーバーにValkeyのPub/Subを足す
